...
export const placeOrder = async (req, res) => {
    try {
        ...
        if (!deliveryAddress.text || !deliveryAddress.latitude || !deliveryAddress.longitude) {
            return res.status(400).json({ message: "Can't deliver to this location" })
        }
    ...
    try {
        ...
        if (status == "out of delivery" && !shopOrder.assignment) {
            ...
            if (candidates.length == 0) {
                await order.save()
                return res.json({
                    message: "Order status updated but there is no delivery boys available now!"
                })
            }
...
export const getDeliveryBoyAssignment = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: `Get assignment order error ${error}` })
    }
}
...
export const acceptOrder = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            message: 'Order accepted by Deliveryboy'
        })
...
export const sendDeliveryOtp = async (req, res) => {
    try {
        ...
        if (!order || !shopOrder) {
            return res.status(400).json({ message: "Enter valid shop order ID" })
        }
