    ...
    return (
        <div className='min-h-screen flex items-center justify-center p-4' style={{ backgroundColor: bgColor }}>
            <div className={`bg-white rounded-xl shadow-lg w-full max-w-md p-8 border`} style={{
                ...
                <p className='text-gray-600 mb-8'>Log in to your account and get delicious food deliveries.</p>
                ...
                px-4 py-2 duration-200 bg-[#ff4d2d] hover:scale-102 font-semibold transition-transform text-white hover:bg-[#e64323]`} onClick={handleSignIn}>
                ...
                px-4 py-2 hover:scale-102 transition-transform font-medium bg-gray-200 hover:bg-gray-300' onClick={handleGoogleAuth}>
                    <FcGoogle size={20} />
                    <span>{loading ? <Loader2 className='w-5 h-5 animate-spin' /> : "Sign in with Google"}</span>
                </button>
                <p className='text-center mt-4 cursor-pointer' onClick={() => navigate("/signup")}>Don't have an account ?{" "}
                    <span className='text-[#ff4d2d] font-medium hover:underline hover:scale-105 transition-transform'>Sign Up</span></p>
            </div>
        </div>
    )
}
